...
class Publications extends React.Component {
  ...
  async fetchDBLPdata() {
    const promiseArray = this.state.professors.map(prof => {
    ...
    for (let element of data) {
      flattenedData = [...flattenedData, ...element]
    }
    saveJSONToFile(flattenedData)
    ...
    if (this.state.loadedCount < this.state.professors.length)
      ...
      return (
        <Layout mainClass="homepage" title="Homepage">
          ...
          <Tabs defaultActiveKey={"Research Highlights"} className="mb-3">
            <Tab eventKey={"Research Highlights"} title={"Research Highlights"}>
              <MasonryCardGrid
